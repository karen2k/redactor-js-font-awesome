(function ($) {
    $.Redactor.prototype.awesome = function () {
        return {
            init: function () {
                ...
                $(document).on('click', '#'+that.awesome.searchInput, function(){
                    return false;
                });
                $(document).on('keyup', '#'+that.awesome.searchInput, function () {
                    var searchFor = $(this).val();
                    var showHide;
                    var isVisible;
                    var $icon;
                    if (searchFor.length < 1) {
                        $('.redactor-dropdown-box-awesome a').removeClass('visible');
                        return;
                    }
                    for (var i = 0; i < that.awesome.icons.length; i++) {
                        showHide = (that.awesome.icons[i].indexOf(searchFor) > -1);
                        $icon = $('#redactor-fa-' + that.awesome.icons[i]).parent();
                        isVisible = $icon.hasClass('visible');
                        (showHide != isVisible) ? $icon.show() : $icon.hide();
                    }
                });

                dropdown['search'] = {
                    title: '<input type="text" id="'+that.awesome.searchInput+'" placeholder="Start typing icon name">',
                    func: function () {
                        e.preventDefault();
                        this.button.setActive('awesome');
                        return false;
                    }
                };

                $.each(that.awesome.icons, function (i, s) {
                    dropdown['s' + i] = {
                        title: '<i class="fa fa-' + s + '" id="redactor-fa-' + s + '"></i>&nbsp;&nbsp;<span>' + s + '</span>',
                        func: function () {
                            that.awesome.insertAwesome(s);
                        }
                    };
                });
                ...
                this.button.setAwesome('awesome', 'fa-flag-o');
                this.button.addDropdown(button, dropdown);
            },
            searchInput: 'redactor-awesome-search',
            insertAwesome: function (s) {
                var html = '&nbsp;<i class="fa fa-' + s + '">&nbsp;</i>&nbsp;';
